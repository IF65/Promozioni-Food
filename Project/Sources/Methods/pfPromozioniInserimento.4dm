...
Case of 
	...
	: ($azione="inizializzaArticoli")
		...
		ARRAY REAL:C219(arAR_importo; 0)  //9
	...
	: ($azione="aggiungiRigaArticoli")
		...
		INSERT IN ARRAY:C227(arAR_importo; 1)
	...
	: ($azione="eliminaRigaArticoli")
		If (Count parameters:C259>=2) & ($2<=Size of array:C274(arAR_id))
			...
			DELETE FROM ARRAY:C228(arAR_importo; $2)
		End if 
			...
			If ($httpResponse=200) & (<>error=0)
				...
				If (Size of array:C274($promozioni)=1)
					...
					For ($i; 1; Size of array:C274($articoli))
						...
						APPEND TO ARRAY:C911(arAR_importo; OB Get:C1224($articoli{$i}; "importo"; Is real:K8:4))
					End for 
					SORT ARRAY:C229(arAR_codiceReparto; arAR_barcode; arAR_codiceArticolo; arAR_descrizione; arAR_molteplicita; arAR_idPromozioni; arAR_gruppo; arAR_importo; arAR_id; >)
	...
	: ($azione="salvaPromozione")
		...
		For ($i; 1; Size of array:C274(arAR_id))
			...
			OB SET:C1220($articolo; "importo"; arAR_importo{$i})
