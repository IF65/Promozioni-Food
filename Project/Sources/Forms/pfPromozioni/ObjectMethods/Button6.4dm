...
If (Size of array:C274($selezione)>0)
	...
	For ($i; 1; Size of array:C274($id))
		...
		Case of 
			...
			: ($tipo{$i}="0486")
				$nomeFile:="PROMO_0486_"
			: ($tipo{$i}="0487")
				$nomeFile:="PROMO_0487_"
			: ($tipo{$i}="0492")
		...
		For ($j; 1; Size of array:C274($sedi))
			...
			If (True:C214)
				...
				$usedExtension:=".DAT"
				If ($tipo{$i}="REGN") | ($tipo{$i}="ACPT") | ($tipo{$i}="EMBU") | ($tipo{$i}="REBU") | ($tipo{$i}="COUP") | ($tipo{$i}="DELC") | ($tipo{$i}="DELV") | ($tipo{$i}="DELP") | ($pmt{$i})
